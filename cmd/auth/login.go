...
MESHSTACK_WORKSPACE already says. An API key login asks the same way, while --apitoken asks nothing.`,
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			// A token given with --apitoken was minted elsewhere and already names the workspace it
			// belongs to, so this login neither asks for one nor can list any: a building block
			// runner's token, the usual reason to pass one, may list no workspace at all.
...
// is a fallback source, so --workspace and MESHSTACK_WORKSPACE are taken as given and nothing is
// printed, while the profile's default ranks below it: a login is what changes that default.
// Unlike the secret prompts above it needs no --stdin, as a workspace is nothing to hide and the
// list has to be shown for the choice to make sense.
...
// selectWorkspace prints the workspaces as a numbered list and asks for one number until it gets
// one. An abandoned prompt is an error rather than no workspace at all: a login that stored none
// leaves every later command without one, and by then this list is long gone.
func selectWorkspace(ctx context.Context, prompt Prompt, workspaces setting.Workspaces) (*setting.MeshWorkspace, error) {
	...
	// The numbers have to be answerable later, so the listed workspaces are kept in the order they
	// were printed in rather than looked up again.
...
func newPromptingSource(settingEnvKey string, openStdinFlag *internal.Flag[bool], prompt Prompt, what string) setting.FrontendSource {
	...
	return setting.LookupSource(settingEnvKey, description, func(ctx context.Context) (string, error) {
		...
		if answer == "" {
			// An error and not an empty value, which ResolveSetting would take as this source
			// having nothing and go on to read the environment: a token exported there is not
			// what was asked for at this prompt.
