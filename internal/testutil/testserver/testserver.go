...
// so that a test can drive the token cache without a backend.
//
// It imports internal/auth for the api key settings. A package whose own tests live in the
// internal test package therefore cannot use it without creating an import cycle, and has to
// move those tests to an external test package first.
package testserver
...
// goroutine, where require may not be used.
...
func (s *Server) mint(validFor time.Duration) string {
	...
	minted := mintedToken{value: "e30." + claims + ".test-signature", honored: true}
