...
type ClientConfig struct {
	...
	// Both EndSessionEndpoint and RevocationEndpoint are optional in the discovery document, and one of them ends a session.
	EndSessionEndpoint *xurl.URL `json:"end_session_endpoint"`
	RevocationEndpoint *xurl.URL `json:"revocation_endpoint"`
}

// NewClient discovers the identity provider a meshStack delegates its logins to.
...
func (c Client) Refresh(ctx context.Context, refreshToken string, scopes scope.Scopes) (resp Token, err error) {
	...
	if resp.RefreshToken == "" {
		// Keycloak does rotate the refresh token on every call though (also to detect replay attacks)!
...
// EndSession ends the login at the identity provider, as `meshstack auth logout --revoke` asks for.
