...
func WorkspacesFromContext(ctx context.Context) (Workspaces, error) {
	...
	if !found {
		// An error rather than a panic, as a front end reaches this through pkg/setting and a panic
		// there takes its process down. See SetWorkspacesInContext for who provides them.
