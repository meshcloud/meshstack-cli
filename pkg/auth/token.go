...
func (s *Session) mintApiKey(ctx context.Context, credentials profile.Credentials, tokenScope scope.Scope) (profile.Credentials, credential.IssuedToken, error) {
	...
	if apiKey == nil || apiKey.Id == "" {
		return credentials, credential.IssuedToken{}, errors.New("no meshStack API key: this profile's current method is an API key, but it holds no key id. Run `meshstack auth login --api-key=<id>`")
	}
