...
func (s *Session) loginDevLocal(ctx context.Context, result *LoginResult) error {
	...
	if dev == nil {
		return fmt.Errorf("this meshStack publishes no local dev credentials: %s serves no devLocalCredentials in /mesh/info. meshStack publishes them only on a local dev stack, so --dev-local cannot bootstrap anything here. Use `meshstack login --api-key=<id>` with a key issued in meshPanel instead",
			s.Endpoint)
	}
