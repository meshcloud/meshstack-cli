...
func (s *Session) loginWithBrowser(ctx context.Context, options LoginOptions, result *LoginResult) error {
	...
	if s.noInput {
		return fmt.Errorf("cannot wait for a browser login: %s says nobody is here to visit the login URL. Use `meshstack auth login --api-key=<id>` instead", tty.NoInput.EnvKey())
	}
...
func (s *Session) loginWithApiKey(ctx context.Context, result *LoginResult) error {
	...
	if offered == nil || offered.Id == "" {
		return fmt.Errorf("no API key id: name one with --api-key=<id> or %s. The id is the only part of a credential that may appear on a command line",
