...
- A command that works on a meshObject is named after **the kind, flat and in full** —
  `meshstack buildingblockdefinitionversion list`, not a `definition version` nesting the API does
  not have either. It carries a **short alias**, `bbdv`, so the full name stays typable.
</rules>
